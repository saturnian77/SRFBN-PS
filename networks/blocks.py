...
class conv_KFNxN(nn.Module):
    def __init__(self, ch_in, ch_out, num, matrix, kernel_size,stride,padding):
        super().__init__()
        self.ch_in=ch_in
        self.ch_out=ch_out
        self.matrix = matrix
        ...
        self.stride=stride
        self.element = nn.Parameter(torch.randn(ch_out, num), requires_grad=True) #3x3x64x1    
        if self.init == True:
            self.initialize()
    ...
    def forward(self, x):
        self.kernel = rearrange(torch.matmul(self.element,self.matrix), 'cout (cin k1 k2) -> cout cin k1 k2', cin = self.ch_in, cout = self.ch_out, k1=self.kernel_size, k2=self.kernel_size)
        out = F.conv2d(input = x, weight = self.kernel, bias = None, stride = self.stride, padding = self.padding)
        return out

def ConvBlock_us(in_channels, out_channels, banklength, matrix, kernel_size, stride=1, dilation=1, bias=True, valid_padding=True, padding=0,\
              act_type='relu', norm_type='bn', pad_type='zero', mode='CNA'):
    assert (mode in ['CNA', 'NAC']), '[ERROR] Wrong mode in [%s]!'%sys.modules[__name__]
    ...
    if valid_padding:
        ...
        pass
    p = pad(pad_type, padding) if pad_type and pad_type != 'zero' else None
    conv = conv_KFNxN(in_channels, out_channels, banklength, matrix, kernel_size, stride=stride, padding=padding)
