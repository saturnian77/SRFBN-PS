...
import numpy as np
from .blocks import ConvBlock, ConvBlock_us, DeconvBlock, MeanShift

class FeedbackBlock_us(nn.Module):
    def __init__(self, num_features, num_groups, num_steps, upscale_factor, act_type, norm_type, banklength, filterbank):
        super(FeedbackBlock_us, self).__init__()
        if upscale_factor == 2:
            stride = 2
            padding = 2
            kernel_size = 6
        ...
        for idx in range(self.num_groups):
            self.upBlocks.append(DeconvBlock(num_features, num_features,
                                             ...
                                             act_type=act_type, norm_type=norm_type))
            for idxx in range(self.num_steps):                                
                self.downBlocks.append(ConvBlock_us(num_features, num_features, banklength, filterbank,
                                                 kernel_size=kernel_size, stride=stride, padding=padding,
    ...
    def forward(self, x, tstep):
        ...
        for idx in range(self.num_groups):
            ...
            LD_L = self.downBlocks[3*tstep+idx](LD_H)
...
class SRFBN(nn.Module):
    def __init__(self, in_channels, out_channels, num_features, num_steps, num_groups, upscale_factor, act_type = 'prelu', norm_type = None):
        ...
        filt_num = np.round(num_groups*num_features*gamma).astype(int) # 3x3x64x1 kernel partitioning
        self.filterbank = nn.Parameter(torch.randn(filt_num,kernel_size*kernel_size*num_features), requires_grad=True) # 3x3x64 kernel partitioning
        ...
        # basic block
        self.block = FeedbackBlock_us(num_features, num_groups, num_steps, upscale_factor, act_type, norm_type, filt_num, self.filterbank)
